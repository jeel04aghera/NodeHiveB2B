-- +goose Up
-- Seed the built-in template registry (org_id NULL = available to every org).
-- These were previously only in scripts/seed_templates.sql, which prod never ran —
-- so the templates table was empty in Railway and the Deploy Workload modal's
-- template dropdown hung on "Loading…". Migrations auto-run on startup, so making
-- this a migration guarantees the built-ins exist in every environment.
-- Idempotent via the idx_templates_global_name partial unique index (name WHERE org_id IS NULL).
...
-- +goose Down
-- +goose StatementBegin
DELETE FROM templates WHERE org_id IS NULL AND name IN (
  'Ubuntu 22.04', 'Python 3.11', 'PyTorch 2.1 (CUDA 12.1)',
  'TensorFlow 2.15 (GPU)', 'CUDA 12.2 Devel', 'JupyterLab Data Science'
);
-- +goose StatementEnd
